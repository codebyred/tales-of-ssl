use crate::{models::Problem, utility::get_iso_country_name_from};
use anyhow::Context;
use openssl::{
    asn1, base64,
    bn::BigNum,
...
impl CertificateBuilder {
    pub fn build(problem: Problem) -> anyhow::Result<Certificate> {
        let key_der_format = base64::decode_block(&problem.private_key)
            .context("decoding private key with base64 decoder")?;
        let rsa = Rsa::private_key_from_der(&key_der_format[..])
            .context("creating public key form der format key")?;
        ...
        name_builder
            .append_entry_by_nid(Nid::COMMONNAME, &problem.required_data.domain)
            .context("appending domain to name_builder")?;
        name_builder
            .append_entry_by_nid(Nid::COUNTRYNAME, &iso_country_name)
            .context("appending country to name_builder")?;
        ...
        builder
            ...
            .context("setting x.509 subject name")?;
        builder
            .set_issuer_name(&name)
            .context("setting x.509 issuer name")?;
        ...
        builder
            ...
            .context("setting x.509 serial number")?;
        ...
        builder
            ...
            .context("siging the certificate with private key")?;
